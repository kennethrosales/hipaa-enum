...
require 'hipaa/enum/validators/multiple_validator'
require 'hipaa/enum/configuration'
  ...
  module Enum
    ...
    module ClassMethods
      def enum(name, values, options = {})
        field_name = :"#{Hipaa::Enum.configuration.field_name_prefix}#{name}"
      ...
      def create_validations(field_name, values, options)
        if options[:multiple] && options[:validate]
          validates field_name, 'hippa/enum/validators/multiple': { in: values.map(&:to_sym), allow_nil: !options[:required] }
